...
import { CardWithList2, ListWithCards2, SafeBoard2, SafeCardWithList2 } from "@/types";
...
export const getFactors =(x:number):number[]=>{
 ...
 export const findClosestFactor = (input:number,
    ...
    for (const factor of factorsOf100) {
      // 100
      // 100, 50
      //100, 50,25,20
...
export const getCardsFromSafeBoard0 = (data: SafeBoard2): SafeCardWithList2[] => {
  ...
  const allCards: SafeCardWithList2[] = [];
...
export const getCardsFromLists = (data: ListWithCards2[], asc:Boolean=true): CardWithList2[] => {
  // Combine all cards from all lists into a single array
  const allCards: CardWithList2[] = [];
  data.forEach((list) => allCards.push(...list.cards));
  allCards.sort((a,b)=>{ 
     const a_date= new Date(a.updatedAt)  
     const b_date= new Date(b.updatedAt)  
     if (asc==true){
      //start with oldest
...
export function  getTextAreaRow(val:number){
  ...
  const x:number= Math.floor(val/30) + 1
...
export function getTextAreaHeight(val:number){
  ...
  // a
  const x:number= Math.floor(val/30)*30 + 30 
...
export function isJsonStringEditorCompatible(str: unknown): boolean {
  if (typeof str !== 'string') return false;

  try {
    const json = JSON.parse(str);
    
    // Check if the parsed object is actually an object and not null
    if (typeof json === 'object' && json !== null) {
      // Check for Editor.js 'blocks' or similar structures
      if ('blocks' in json || 'blockMap' in json) {
        return true;
      }
    }
    
    return false; // Return false if it's valid JSON but doesn't have editor blocks
  } catch (e) {
    return false;
  }
}
...
export function getProgressStatus(b: ProgressData): string {
  // Defensive check: prevent division by zero if cardCount is 0
  const percentage = b.cardCount > 0 
    ? Math.round((b.completed * 100) / b.cardCount) 
    : 0;

  const completedStr = getGreatThanZeroOrEmptyStr(
    b.completed,
    `, comp=${b.completed}(${percentage}%)`
  );
